...
function buildTiles(film: Film): Tile[] {
  const tiles: Tile[] = [];
  if (film.trailer) {
    tiles.push({
      number: 0,
      title: "Trailer",
      duration: film.trailer.duration,
      caption: film.trailer.caption,
      kind: "trailer",
      released: true,
    });
  }
...
/* -------------------------------------------------------------------------- */
/* Public component — coordinates player + grid                               */
/* -------------------------------------------------------------------------- */
...
export function FilmStage({ film }: { film: Film }) {
  ...
  const selectedTile = tiles.find((t) => t.number === selectedNumber) ?? initialTile;
  const releasedSceneCount = film.scenes.filter((s) => s.released).length;

  return (
    <section className="grid gap-8 pt-6 lg:grid-cols-[0.42fr_0.58fr] lg:items-start">
      <PlayerColumn film={film} tile={selectedTile} />
      <DetailColumn
        film={film}
        tiles={tiles}
...
function PlayerColumn({ film, tile }: { film: Film; tile: Tile }) {
  const hudLabel =
    tile.kind === "trailer"
      ? `Trailer · ${tile.duration}`
      : `Micro scene ${String(tile.number).padStart(2, "0")} · ${tile.duration}`;

  const bottomLine = tile.kind === "trailer" ? film.title : tile.title;

  const playAriaLabel =
    tile.kind === "trailer"
      ? "Play trailer"
      : `Play micro scene ${tile.number}`;
        ...
        <div className="overflow-hidden rounded-[1.05rem] bg-[var(--ink)] p-1">
          ...
          >
            ...
            <div className="relative z-10 mb-24 w-full px-6 text-center">
              <p
                key={tile.number}
                className="serif text-2xl leading-snug text-white drop-shadow-[0_2px_24px_rgba(0,0,0,0.6)] transition-opacity duration-300 sm:text-3xl"
              >
                {tile.caption}
              </p>
            </div>
    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="mt-3 font-mono text-[11px] uppercase tracking-[0.22em] text-[var(--foreground)]/50">
          {film.totalDuration} · {totalScenes} micro scenes · {releasedSceneCount} out today
        </p>
      </header>
      ...
      <SceneGrid
        ...
        releasedCount={releasedSceneCount}
        hasTrailer={Boolean(film.trailer)}
      />
    </div>
  );
}

function ActionRow({ stats }: { stats: { likes: string; saves: string } }) {
  return (
    <section
      ...
      <header className="flex items-end justify-between">
        ...
        >
          <span className="rounded-full bg-[var(--paper)] px-3 py-1 text-xs font-medium text-[var(--ink)]">
            {hasTrailer ? "00" : "01"} — {String(totalScenes).padStart(2, "0")}
          </span>
        </nav>
      </header>
...
function SceneTile({
  ...
  // Visual highlight when this tile is the one in the player.
  const stateClasses = selected
    ? "border-[var(--amber)]/55 bg-[radial-gradient(circle_at_30%_20%,rgba(232,184,106,0.25),transparent_55%),linear-gradient(180deg,#1a1612,#0c0a08)] text-[var(--foreground)] ring-1 ring-[var(--amber)]/40"
    : tile.kind === "trailer"
      ...
      : tile.released
        ...
        : "border-white/10 bg-[var(--background)] text-[var(--foreground)]/55 hover:border-white/25";

  const ariaLabel =
    tile.kind === "trailer"
      ? `Play trailer (${tile.duration})`
      : `Play micro scene ${tile.number}: ${tile.title}`;
...
function renderBadge(tile: Tile, selected: boolean) {
  // Trailer keeps its identity badge regardless of selection.
  if (tile.kind === "trailer") {
    return (
  ...
  // Selected scene wears the "Now" badge.
  if (selected) {
    return (
